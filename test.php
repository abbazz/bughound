		...
		$sevErr = "Severity is required";
		  ...
		  if((empty($_POST["probsum"]) && !empty($_POST["prob"])) ||(empty($_POST["prob"]) && !empty($_POST["probsum"])) || (empty($_POST["prob"]) && !empty($_POST["probsum"]))){
			...
			$probErr = "Both of these fields are required";
					...
					$query1 = "INSERT INTO bugs (program_type, report_type, severity, prob_summary, problem, reported_by, report_date) VALUES ('$program_type','$report_type','$severity','$prob_summary','$problem','$reported_by', '$rep_date ');";
