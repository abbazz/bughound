...
<html>
    ...
    <body class = "centered-wrapper" bgcolor = "#FFD700">
        <?php
			...
			$query1 = "SELECT * FROM areas where program_name = '$search_prog_name'" ;
			}
			$result = mysqli_query($con, $query1);
            echo "<table border=1 ><th>AID</th><th>program name</th><th>Area name</th>\n";
            $none = 0;
            while($rows=mysqli_fetch_row($result)) {
                $none=1;
				echo "<tr><td><a href = 'updatearea.php?value_key=$rows[0]'>".$rows[0]."</td> <td>".$rows[1]."</td> <td>".$rows[2]."</td>" ;
			}
		...
		<script language=Javascript>
            function go_home(){
                window.location.replace("areaedit.php");
            }
        </script>
	</body>
</html>